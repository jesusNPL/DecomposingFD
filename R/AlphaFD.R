...
# Load data
# tdmat <- read.csv("Data/tussock.traits.csv", row.names = 1)
# tdmat <- FD::gowdis(tdmat)

# cdm <- read.csv("Data/tussock.comms.csv", row.names = 1)

# Run Scheiner's FD

# FTD.comm(tdmat = tdmat, # trait distance matrix
#        spmat = cdm, # community data matrix
#       q = 1, # q order = [0 = richness; 1 = Shannon's H; 2 = Simpson's D]
#      abund = TRUE) # abundance weighted
